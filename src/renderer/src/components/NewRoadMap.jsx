...
const InteractiveRoadmap = () => {
  ...
  useEffect(() => {
    const height = roadmapData.reduce((acc, task, index) => {
      ...
      const subtasksHeight = showSubtasksForThis && task.subTasks ?
        SUBTASK_HEIGHT * task.subTasks.length + SUBTASK_GAP : 0;
  ...
  const renderTask = (task, yPosition, showSubtasksForThis) => {
    ...
    return (
      <g key={task.id}>
        ...
        {/* Milestone markers */}
        {task.singleDates && task.singleDates.map((milestone, i) => (
          <g key={i}>
            <line
        ...
        {task.overlaps && task.overlaps.map((overlap, i) => {
              ...
              <rect
                ...
                onMouseEnter={(e) => {
                  ...
                  setTooltip({
                    ...
                    content: (
                      <div>
                        <div><strong>Resource Overlap</strong></div>
        ...
        {task.subTasks && showSubtasksForThis && task.subTasks.map((subtask, i) => {
                  ...
                  setTooltip({
                    ...
                    content: (
                      <div>
                        ...
                        {subtask.developer && <div>Developer: {subtask.developer.join(', ')}</div>}
                      </div>
                    )
                  });
                }}
                    ...
                    <Group transform={zoom.toString()}>
                      ...
                      {roadmapData.reduce((elements, task, index) => {
                        ...
                        for (let i = 0; i < index; i++) {
                          ...
                          const previousSubtasksHeight = previousSubtasksVisible && previousTask.subTasks ?
                            SUBTASK_HEIGHT * previousTask.subTasks.length + SUBTASK_GAP : 0;
