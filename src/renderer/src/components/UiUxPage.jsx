...
const UIUXRequestTableRow = memo(({ request, onUpdate, onDelete }) => {
  ...
  const handleDelete = useCallback(() => onDelete(request.id), [request.id, onDelete])

  const handleStepChange = useCallback(
    (step) => onUpdate(request.id, { step }),
    [request.id, onUpdate]
  )
...
const UiUxManagementDashboard = () => {
  ...
  const filteredRequests = useMemo(() => {
    return uiuxRequests
      ...
      .sort((a, b) => {
        const priorityIndexA = a.priority || 99
        const priorityIndexB = b.priority || 99
    ...
    return (
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        ...
        <StatCard
          ...
          percentage={
            stats.totalRequests ? Math.round((stats.inProgress / stats.totalRequests) * 100) : 0
          }
        />
        <StatCard
          title="Completed"
        ...
        <StatCard
          ...
          percentage={
            stats.totalRequests
              ? Math.round((stats.priority.critical / stats.totalRequests) * 100)
              : 0
          }
        />
      </div>
