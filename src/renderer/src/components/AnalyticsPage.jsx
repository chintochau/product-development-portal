...
const AnalyticsPage = () => {
  ...
  return (
    <FrameWraper>
      ...
      <PlanningDetails selectedPlan={selectedPlan} />
    </FrameWraper>
  )
}
...
const PlanningDetails = ({ selectedPlan }) => {
  const { milestones } = useSingleProduct()
  ...
  const milestoneName = getNameForProject(milestoneProjectId) + milestones.find(
...
const CommitAnalytics = forwardRef(({ ticket }, ref) => {
  ...
  // Parse commit dates
  const firstCommitDate = commits.length > 0 ? dayjs(commits[0].created_at) : getFromAnalytics('firstCommitDate')? dayjs(getFromAnalytics('firstCommitDate')) : null
  const lastCommitDate = commits.length > 0 ? dayjs(commits[commits.length - 1].created_at) : getFromAnalytics('lastCommitDate')? dayjs(getFromAnalytics('lastCommitDate')) : null
  const closedDate = closed_at ? dayjs(closed_at) : getFromAnalytics('closedDate')? dayjs(getFromAnalytics('closedDate')) : null

  // Parse label history
  const labelHistory = labels
    .filter((label) => label?.label?.name?.startsWith('workflow::'))
    .map((label) => ({
      ...label,
  ...
  const commitsByAuthor = commits.length > 0 ? commits.reduce((acc, commit) => {
    const author = commit.author?.name || 'Unknown'
    acc[author] = (acc[author] || 0) + 1
    return acc
  }, {})  : getFromAnalytics('commitsByAuthor') ? getFromAnalytics('commitsByAuthor') : {}
  ...
  const cycleCount = labelHistory.filter(
    (label) => label.labelName === 'workflow:: 2 doing' && label.action === 'add'
  ).length

  const totalCommits = commits.length || getFromAnalytics('totalCommits')

  const totalTimeInDoing = totalTimeInLabel('workflow:: 2 doing') === "N/A" ? getFromAnalytics('totalTimeInDoing') : totalTimeInLabel('workflow:: 2 doing')
  const totalTimeInReview = totalTimeInLabel('workflow:: 3 review') === "N/A" ? getFromAnalytics('totalTimeInReview') : totalTimeInLabel('workflow:: 3 review')
  const totalTimeInTesting = totalTimeInLabel('workflow:: 4 testing') === "N/A" ? getFromAnalytics('totalTimeInTesting') : totalTimeInLabel('workflow:: 4 testing')
  const percentageOfTimeInDoing = percentageOfLabelTime('workflow:: 2 doing') === "" ? getFromAnalytics('percentageOfTimeInDoing') : percentageOfLabelTime('workflow:: 2 doing')
  const percentageOfTimeInReview = percentageOfLabelTime('workflow:: 3 review') === "" ? getFromAnalytics('percentageOfTimeInReview') : percentageOfLabelTime('workflow:: 3 review')
  const percentageOfTimeInTesting = percentageOfLabelTime('workflow:: 4 testing') === "" ? getFromAnalytics('percentageOfTimeInTesting') : percentageOfLabelTime('workflow:: 4 testing')
      ...
      <div className="flex gap-6">
        ...
        <div className="flex-1 space-y-1">
          ...
          <div className="mt-6">
            <h4 className="text-lg font-semibold text-gray-800 mb-2">
              Commits: <span className="text-blue-600">{totalCommits}</span>
            </h4>
        ...
        <div className="flex-1 space-y-1">
          ...
          <div className="flex justify-between items-center">
            ...
            <span className="font-medium text-gray-900">{cycleCount}</span>
          </div>
        </div>
      </div>
    </div>
